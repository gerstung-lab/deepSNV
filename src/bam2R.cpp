...
int bam2R(char** bamfile, char** ref, int* beg, int* end, int* counts, int* q, int* s, int* head_clip, int* maxdepth, int* verbose, int* mask)
{
  ...
  int tid, pos, n_plp = -1;
	...
	if (strcmp(*ref, "") == 0) { // if a region is not specified
		...
		while((ret = sam_read1(nttable.in, head, b)) >= 0){
			if ((b->core.flag & *mask)==0) { bam_plp_push(buf, b); };
