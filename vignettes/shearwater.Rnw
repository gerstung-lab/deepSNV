...
opts_chunk$set(width=3,fig.height=3, pointsize=6)
    options(width=60)
	options(continue="  ")
@

%\VignetteIndexEntry{Subclonal variant calling with multiple samples and prior knowledge}
...
<<fig.width=5, fig.height=5, pointsize=8>>=
library(deepSNV)
library(RColorBrewer)
n <-  100
n_samples <- 1000
x <-  0:20
y <-  cbind(rep(x, each = length(x)), rep(x, length(x)))
par(bty="n", mgp = c(2,.5,0), mar=c(3,3,2,2)+.1, las=1, tcl=-.33, mfrow=c(2,2))
for(nu in 10^c(-4,-2)){
	counts = aperm(array(c(rep(round(n_samples*n* c(nu,1-nu,nu,1-nu)), each=nrow(y)), cbind(n - y, y)[,c(3,1,4,2)]), 
					dim=c(nrow(y) ,4,2)), c(3,1,2))
	for(rho in c(1e-4, 1e-2)){
		...
		image(z=log10(matrix(BF[2,,1], nrow=length(x))), 
				...
				font.main=1)
		text(y[,1],y[,2],ceiling(log10(matrix(BF[2,,1], nrow=length(x)))), cex=0.5)
	}
}
@
...
for(nu in 10^c(-4,-2)){
	...
	for(rho in c(1e-4, 1e-2)){
		...
		image(z=log10(matrix(BF[2,,1], nrow=length(x))), 
				x=x, 
				y=x, 
				breaks=c(-100,-8:0), col=rev(brewer.pal(9,"Reds")), 
				xlab = "Forward allele count", 
				ylab="Backward allele count", 
				main = paste("rho =", format(rho, digits=2), "nu = ", format(nu, digits=2)), 
				font.main=1)
