...
#' Faster C version of beta binomial
#' @param x Counts
#' @param n Coverage
#' @param mu Relative frequency (scaled by dispersion)
#' @param disp Dispersion factor rho/(1-rho)
                 ...
                 mu.max = 1 - mu.min) # max error rate
{
  ...
  if (return.value == "err") { # to return error
  ...
  if(model %in% c("OR","adaptive")){
    ...
    Bf.fw <- fast_logbb(x.fw, n.fw, nu0.fw, rdisp) +
      fast_logbb(x.bw, n.bw, mu0.bw, rdisp) +
      fast_logbb(X.fw, N.fw, nu0.fw, rdisp) -
      fast_logbb(x.fw, n.fw, mu, rdisp) -
      fast_logbb(x.bw, n.bw, mu, rdisp) -
      fast_logbb(X.fw, N.fw, nu.fw, rdisp)
    Bf.fw = exp(Bf.fw)

    Bf.both = fast_logbb(x.fw, n.fw, nu0.fw, rdisp) +
      fast_logbb(X.fw, N.fw, nu0.fw, rdisp) -
      fast_logbb(x.fw, n.fw, mu, rdisp) -
      fast_logbb(X.fw, N.fw, nu.fw, rdisp)

    ## Bayes factor reverse
    Bf.bw <- fast_logbb(x.fw, n.fw, mu0.fw, rdisp) +
      fast_logbb(x.bw, n.bw, nu0.bw, rdisp) +
      fast_logbb(X.bw, N.bw, nu0.bw, rdisp) -
      fast_logbb(x.fw, n.fw, mu, rdisp) -
      fast_logbb(x.bw, n.bw, mu, rdisp) -
  ...
  if (model == "adaptive") { # if in adaptive mode then switch to AND model if below min.cov or above max.odds
    if (!is.null(min.cov))
  ...
  cons = apply(X, 1, which.max) # mutation type with hughest counts for each site

  for (i in 1:ncol(Bf)) Bf[, i, cons[i]] = NA # for each site set BF to NA for the mutation type with highest count (???)

  if (return.value == "P0") {
    return(Bf/(1 + Bf)) # return posterior probability
  }
  else {
    return(Bf) # return bayesian factor
  }
